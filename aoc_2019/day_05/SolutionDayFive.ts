...
export class SolutionDayTwo {
    ...
    readOpCode(): OpCode {
        let positions = this.numbers.slice(this.currentIndex + 1, this.currentIndex + )
    ...
    executeOpCode(opCode) {
        switch (opCode.operationValue) {
            ...
            case 3: {
                this.updateNumbers(opCode.updateIndex, opCode.updateIndex)
                break;
            }
            case 4: {
                this.updateIndex(opCode.updateIndex)
                break;
            }
            case 99: {
